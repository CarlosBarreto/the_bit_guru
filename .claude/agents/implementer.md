...
5. **Escribe los tests** que validan los criterios de `acceptance` en
   `frontend-astro/tests/`.
6. **Verifica** ejecutando `.\init.ps1`. Si falla → vuelve al paso 4.
...
- Si una herramienta falla de manera inesperada, NO improvises un workaround.
  Para, anota en `progress/current.md` con estado `blocked`, y termina la sesión.
